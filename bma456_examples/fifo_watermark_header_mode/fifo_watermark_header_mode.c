...
#define BMA456_FIFO_RAW_DATA_BUFFER_SIZE  UINT16_C(1024)

/*! Length of data to be read from FIFO */
#define BMA456_FIFO_RAW_DATA_USER_LENGTH  UINT16_C(1024)

/*! Setting a watermark level in FIFO */
#define BMA456_FIFO_WATERMARK_LEVEL       UINT16_C(650)
 ...
 */
#define BMA456_FIFO_ACCEL_FRAME_COUNT     UINT8_C(100)
...
int main(void)
{
     ...
     */
    uint8_t fifo_data[BMA456_FIFO_RAW_DATA_USER_LENGTH + BMA4_SENSORTIME_OVERHEAD_BYTE] = { 0 };

    /* Array of accelerometer frames -> Total bytes =
     * 100 * (6 axes bytes(+/- x,y,z) + 1 header byte) = 700 bytes */
    struct bma4_accel fifo_accel_data[BMA456_FIFO_ACCEL_FRAME_COUNT] = { { 0 } };
    ...
    fifoframe.length = BMA456_FIFO_RAW_DATA_USER_LENGTH + BMA4_SENSORTIME_OVERHEAD_BYTE;

    printf("FIFO is configured in header mode\n");

    rslt = bma456_map_interrupt(BMA4_INTR1_MAP, BMA4_FIFO_WM_INT, BMA4_ENABLE, &dev);
    bma4_error_codes_print_result("bma4_map_interrupt status", rslt);

    wm_lvl = BMA456_FIFO_WATERMARK_LEVEL;
    ...
    while (loop <= 10)
    {
        ...
        if ((rslt == BMA4_OK) && (int_status & BMA4_FIFO_WM_INT))
        {
            ...
            accel_length = BMA456_FIFO_ACCEL_FRAME_COUNT;
